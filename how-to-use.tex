...
\begin{enumerate}[leftmargin=1.5em, itemsep=3pt, topsep=1.5pt]
    ...
    \begin{itemize}[leftmargin=1.2em, itemsep=0.5pt]
        ...
        \item Each slice view shows the \textbf{original} scan for that plane (axial, coronal, sagittal), not a reconstruction.
    \end{itemize}

    \item \textbf{Draw Bounding Boxes}
    \begin{itemize}[leftmargin=1.2em, itemsep=0.5pt]
        \item Press key \textbf{\fbox{\textbf{B}}} on your keyboard (or click \textbf{+ Add Bounding Box}).
        ...
        \item For small nodules on axial, tick \textbf{Axial view: thin slices (TS)} to switch that view from 3\,mm to thin slices (coronal and sagittal stay on 3\,mm).
    ...
    \begin{itemize}[leftmargin=1.2em, itemsep=0.5pt]
        ...
        \item Each box is saved as its own \texttt{.mrk.json} file inside a folder named after the case ID.
...
\textbf{Axial view: thin slices (TS)} & Axial viewer only: 3\,mm $\leftrightarrow$ thin slices (disabled if no TS file) \\
\bottomrule
\end{tabular}
\end{center}
...
\hrule
\vspace{0.12cm}
{\footnotesize\color{gray} Outputs are saved to: \texttt{<Output>/<case\_id>/<case\_id>\_box\_01.mrk.json} with summary in \texttt{\_annotation\_done.json}.}
